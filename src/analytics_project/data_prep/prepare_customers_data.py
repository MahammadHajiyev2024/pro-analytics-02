...
def handle_missing_values(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['ShoppingFrequency'].fillna('Unknown', inplace=True)
    df['NumberOfPurshases'].fillna(0, inplace=True)
...
def main() -> None:
    ...
    # Ensure consistent formatting: capitalize Region and ShoppingFrequency values
    for col in ["Region", "ShoppingFrequency"]:
        if col in df.columns:
            df[col] = df[col].astype(str).str.capitalize()
            logger.info(f"Formatted {col} values to capitalize first letter.")
